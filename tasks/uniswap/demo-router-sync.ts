...
task(
  ...
  const CurrencyJsonAbi = CurrencyJson.abi as Abi;
  ...
  // 2. MINT CURRENCIES
  console.log(`Minting ${mintAmount} Currency0 to wallet ${wallet.address}...`);
  ...
  console.log(`Minting ${mintAmount} Currency1 to wallet ${wallet.address}...`);
  await mintAndSendCurrency({
    wallet,
  ...
  const hash3 = await router.write.removeLiquiditySync(
    [pairAddress, wallet.address, 100, 100],
    {
      tokens: [
        {
          id: pairAddress,
